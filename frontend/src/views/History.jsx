...
export default function History() {
  ...
  return <>
    ...
    {S.workouts.length ? <div className="list">{[...S.workouts].reverse().map(w => <WorkoutRow key={w.id} w={w} onClick={() => workoutDetailSheet(w)} />)}</div>
      : <div className="empty"><div className="ico"><Icon name="history" /></div>
          <div style={{ marginBottom: 10 }}>{t('No workouts yet — finish your first session and it will appear here.')}</div>
          <Button icon="play" variant="primary" onClick={() => nav('/workout')}>{t('Start workout')}</Button></div>}
  </>
}
