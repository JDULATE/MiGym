...
function ExerciseBlock({ entryIdx, compact, onToggle, onField, onAddSet, onRemoveSet, onAddWarmup, onRemoveSetAt, onStartTimed, onPairPrev, onPairNext }) {
  ...
  return <>
    ...
    <div className="card" data-giwi="exercise" style={{ marginTop: 10, marginBottom: 0 }}>
      ...
      {entry.sets.map((s, i) => {
        ...
        return <div key={i} data-giwi={i === 0 ? 'set-row' : undefined}>
          ...
          <div className={'setrow' + (s.done ? ' done' : '') + (col3 ? ' eff3' : '')}>
            ...
            <Check checked={s.done} onChange={() => onToggle(i)}
              aria-label={`${t('Set')} ${phaseNum}${warm ? ' · ' + t('Warm-up') : ''} — ${ex.n}`} />
          </div>
        </div>
      })}
      <div style={{ height: 8 }} />
