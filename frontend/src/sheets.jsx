...
import { Button, ListItem, Slider, Switch, Segmented, SelectRow, Row } from './components/ui.jsx'
  ...
  ui().openSheet(close => <>
    ...
    <div className="list">
      {STARTER_TEMPLATES.map(tpl => {
        ...
        return <ListItem key={tpl.key} onClick={() => {
          close()
          update(st => {
...
function ExerciseDetail({ ex, close }) {
  ...
  return <>
    ...
    {alternatives.length > 0 && <>
      ...
      <div className="list">
        {alternatives.map(a => <ListItem key={a.id} onClick={() => exerciseDetailSheet(a)}>
          <Thumb ex={a} />
...
function AddToRoutine({ ex, close }) {
  ...
  return <>
    ...
    <div className="list">
      {st.routines.map(r => <ListItem key={r.id} onClick={() => pick(r.id)}>
        <span className="lrow-i"><Icon name={glyphOf(r.emoji)} /></span>
        <div className="grow"><div className="tt">{r.name}</div><div className="ss">{exCount(r.ex.length)}</div></div>
        {r.ex.some(e => e.id === ex.id) && <span className="tag">{t('already in')}</span>}<Icon name="plus" className="chev" />
      </ListItem>)}
      <ListItem onClick={() => pick('_new')}><span className="lrow-i" style={{ background: 'var(--surface-3)' }}><Icon name="sparkles" /></span>
        <div className="grow"><div className="tt">{t('New routine')}</div><div className="ss">{t('Create one and start with this exercise')}</div></div><Icon name="plus" className="chev" /></ListItem>
    </div>
  </>
}
...
function ExercisePicker({ onPick, close }) {
  ...
  return <>
    ...
    <div className="list">
      {bp !== '★' && <ListItem onClick={() => customExSheet(null, ex => onPick(ex), q.trim())}>
        <div className="thumb thumb-x"><Icon name="sparkles" /></div>
        <div className="grow"><div className="tt">{t('Create your own exercise')}</div><div className="ss">{t('name + body part, no animation')}</div></div><Icon name="plus" className="chev" />
      </ListItem>}
      {f.slice(0, shown).map(e => <ListItem key={e.id} onClick={() => onPick(e)}>
...
function PlanTools({ close }) {
  ...
  const exportFile = async () => {
    ...
    const name = 'migym-plan-' + todayISO() + '.json'
...
function DayOverride({ iso, close }) {
  ...
  return <>
    ...
    <div className="list">
      {st.routines.map(r => <ListItem key={r.id} onClick={() => set(r.id)}>
        <span className="lrow-i"><Icon name={glyphOf(r.emoji)} /></span>
        <div className="grow"><div className="tt">{r.name}</div><div className="ss">{exCount(r.ex.length)}</div></div>
        {effId === r.id && <Icon name="check" className="accent" />}</ListItem>)}
      <ListItem onClick={() => set('rest')}><span className="lrow-i" style={{ background: 'var(--surface-3)' }}><Icon name="moon" /></span><div className="grow"><div className="tt">{t('Rest / skip this day')}</div></div>{effId === null && <Icon name="check" className="accent" />}</ListItem>
      {hasOvr && <ListItem onClick={() => set('')}><span className="lrow-i" style={{ background: 'var(--surface-3)' }}><Icon name="reset" /></span><div className="grow"><div className="tt">{t('Back to weekly plan')}</div></div></ListItem>}
    </div>
  </>
}
...
function DayAssign({ day, close }) {
  ...
  return <>
    <h3>{t(DAYN[day])}</h3>
    <div className="list">
      <ListItem onClick={() => set('')}><span className="lrow-i" style={{ background: 'var(--surface-3)' }}><Icon name="moon" /></span><div className="grow"><div className="tt">{t('Rest day')}</div></div>{!st.week[day] && <Icon name="check" className="accent" />}</ListItem>
      {st.routines.map(r => <ListItem key={r.id} onClick={() => set(r.id)}>
        <span className="lrow-i"><Icon name={glyphOf(r.emoji)} /></span>
        <div className="grow"><div className="tt">{r.name}</div><div className="ss">{exCount(r.ex.length)}</div></div>
        {st.week[day] === r.id && <Icon name="check" className="accent" />}</ListItem>)}
    </div>
  </>
}
